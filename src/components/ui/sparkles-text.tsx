...
const SparklesText: React.FC<SparklesTextProps> = ({
  text,
  colors = { first: "#22d3ee", second: "#a78bfa" },
  className,
  style,
  sparklesCount = 8,
}) => {
  ...
  useEffect(() => {
    ...
    const initial = Array.from({ length: sparklesCount }, generateStar);
  ...
  return (
    <span
      ...
      style={{ "--sparkles-first-color": colors.first, "--sparkles-second-color": colors.second, ...style } as CSSProperties}
    >
      {sparkles.map(s => (
        <motion.svg
          key={s.id}
