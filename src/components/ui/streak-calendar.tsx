...
function didUseFreezeOnDate(date: Date, periods: StreakPeriod[]): boolean {
  return periods.some((p) => isDateInPeriod(date, p) && p.usedFreeze)
}
...
function getGitCells(endDate: Date, startOfWeek: 0 | 1): { cells: (Date | null)[]; dates: Date[] } {
  ...
  const cells: (Date | null)[] = [...Array.from({ length: leadingEmptyCells }, () => null), ...dates]
  return { cells, dates }
}

const StreakCalendar = React.forwardRef<HTMLDivElement, StreakCalendarProps>(
  ({ className, streak, view = "week", month = new Date(), referenceDate = new Date(), showFreezes = true, startOfWeek = 0, onDayClick, ...props }, ref) => {
    const year = month.getFullYear()
    const monthIndex = month.getMonth()
    const daysInMonth = getDaysInMonth(year, monthIndex)
    const firstDayOfMonth = getFirstDayOfMonth(year, monthIndex)
    const adjustedFirstDay = startOfWeek === 1 ? (firstDayOfMonth + 6) % 7 : firstDayOfMonth
    ...
    gitCells.forEach((date, cellIndex) => {
      ...
      if (!seenGitMonths.has(monthKey) && date.getDate() === 1) {
        ...
        gitMonthLabels.push({ column: Math.floor(cellIndex / 7), label: date.toLocaleDateString("en-US", { month: "short" }) })
      }
    })
    ...
    if (firstDate) {
      ...
      if (!seenGitMonths.has(firstMonthKey)) {
        gitMonthLabels.unshift({ column: 0, label: firstDate.toLocaleDateString("en-US", { month: "short" }) })
      }
    }
    ...
    const gitEndMonthName = today.toLocaleDateString("en-US", { month: "long" })

    const freezeColorStyles = { "--freeze-color": "var(--info)", "--freeze-foreground-color": "var(--info-foreground)" } as React.CSSProperties

    const getCellState = (date: Date) => ({
      isToday: date.getTime() === today.getTime(),
      isFuture: date > today,
      isActive: wasDateActive(date, periods),
      usedFreeze: showFreezes && didUseFreezeOnDate(date, periods),
    })

    return (
      <div ref={ref} role="grid" aria-label={`Streak calendar ${view} view`} className={cn("w-full", view === "month" ? "max-w-sm" : "max-w-3xl", className)} {...props}>
        {view === "month" && (
          <>
            ...
            <div role="row" className="mb-2 grid grid-cols-7 gap-1">
              {weekdays.map((day) => (
                <div key={day} role="columnheader" className="text-muted-foreground text-center text-xs font-medium">{day}</div>
              ))}
            </div>
              ...
              {days.map((day, index) => {
                ...
                return (
                  <button
                    ...
                    disabled={isFuture}
                    className={cn(
                      "relative flex aspect-square items-center justify-center rounded-lg text-sm transition-colors hover:bg-muted p-1",
                      isToday && "ring-primary !bg-primary-foreground !text-primary ring-2 ring-inset",
                      isFuture && "text-muted-foreground/50 cursor-not-allowed",
                      isActive && !usedFreeze && "bg-primary text-primary-foreground hover:bg-primary/90",
                      usedFreeze && "bg-[var(--freeze-color)] text-[var(--freeze-foreground-color)] hover:opacity-90"
                    )}
                ...
                <div key={getDateKey(date)} className="flex flex-col items-center gap-2">
                  <button
                    ...
                    className={cn(
                      ...
                      isFuture && "border-border/40 bg-muted/20 text-muted-foreground/40 cursor-not-allowed",
                      isActive && !usedFreeze && "border-primary bg-primary text-primary-foreground",
                      usedFreeze && "border-[var(--freeze-color)] bg-[var(--freeze-color)] text-[var(--freeze-foreground-color)]",
                  ...
                  >
                    {usedFreeze ? (
                      ...
                      <Check className={cn("h-5 w-5", isToday && "!text-muted-foreground/20")} />
                    ) : null}
                  </button>
                  <span className={cn("text-sm", isFuture ? "text-muted-foreground/50" : "text-foreground")}>{dayLabel}</span>
                </div>
              )
            })}
          </div>
        ...
        {view === "year" && (
          <>
            <div className="mb-4 text-center">
              ...
              <p className="text-muted-foreground text-xs">Ending {gitEndMonthName} {today.getDate()}</p>
            </div>
            <div className="overflow-x-auto">
              <div className="inline-block pr-16">
                <div aria-hidden="true" className="mb-2 grid gap-1" style={{ gridTemplateColumns: gitGridTemplateColumns }}>
                  {gitMonthLabels.map((m) => (
                    <span key={`${m.column}-${m.label}`} className="text-muted-foreground text-xs whitespace-nowrap" style={{ gridColumnStart: m.column + 1 }}>{m.label}</span>
                  ))}
                </div>
                <TooltipProvider>
                  <div role="rowgroup" className="grid grid-flow-col grid-rows-7 gap-1" style={{ gridTemplateColumns: gitGridTemplateColumns }}>
                    {gitCells.map((date, index) => {
                      if (!date) return <div key={`git-empty-${index}`} className="h-4 w-4" />
                        ...
                        <Tooltip key={getDateKey(date)}>
                          <TooltipTrigger asChild>
                            <button
                              ...
                              onClick={() => onDayClick?.(date, isActive)}
                              className={cn(
                                "border-border/40 h-4 w-4 rounded-[0.4rem] border transition-colors hover:ring-ring hover:ring-1",
